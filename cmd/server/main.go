...
func main() {
	...
	if vertexImport != "" {
		...
		// Initialize usage persistence when PostgresStore is available.
		if pgStoreInst != nil {
			pm := usage.NewPersistenceManager(usage.GetRequestStatistics(), pgStoreInst, 5*time.Minute)
			restoreCtx, restoreCancel := context.WithTimeout(context.Background(), 15*time.Second)
			if err := pm.Restore(restoreCtx); err != nil {
				log.WithError(err).Warn("failed to restore usage statistics from database")
			}
			restoreCancel()
			usage.SetGlobalPersistenceManager(pm)
			pm.Start()
			defer pm.Stop()
		}
